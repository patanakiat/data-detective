...
- No prior knowledge of the Pearson coefficient is assumed; *r* is introduced as "how tightly two things move together, from −1 to 1".
...
**1 · Graph framing (3–4 min).** Two fictional teaching groups' maths scores over six terms. The learner changes the vertical axis with presets or sliders. A statistics box shows the means, the final-term difference (1.8 points) and the *gap as drawn in pixels*; only the last one moves. The learner ticks what changed; feedback names any confusion between the picture and the data. A follow-up asks whether the zoomed chart is dishonest; the accepted answer is "it depends — labelled scale, meaningful change", and both extremes get specific counter-examples (a hospital temperature chart; the 25× taller gap).

**2 · Sampling (4–5 min).** A fictional college of 1,200 students; the population's true values are shown because we generated it. The learner designs samples — convenience (leaving the library at 21:00) or simple random (by ID) — at sizes 10/50/200/500, draws repeatedly, and compares each draw with the population line. The table makes the mechanism visible: convenience draws run roughly 55–65% late-library users against a true 25%, and their mean sleep sits about 0.4–0.5 h low **regardless of size**, while random draws wobble at n=10 and converge by n=500. The question "why doesn't a bigger convenience sample fix it?" has the bias answer keyed; the noise answer and the "500 is plenty" answer each get a pointed correction.

**3 · Correlation (4–5 min).** 52 fictional weeks of ice-cream sales and sunburn clinic visits (r = 0.82). The learner colours points by season, then shows the correlation within each season (−0.01, 0.23, 0.26). Four conclusions are offered; the keyed one says the association is real but does not by itself establish causation, with temperature as a plausible common cause. The learner justifies in free text with a self-review checklist and an exemplar. Then the **headline builder**: choose at least two of the three problems and write a fictional misleading headline plus a caption naming the tricks — naming the technique is the transfer task.
...
- Within-season correlations are small but not zero, because temperature still varies inside a season; a sharper analysis would stratify by temperature bands or model temperature directly. This is a good stretch discussion.
