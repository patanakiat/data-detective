...
| `weeks.json` | `buildWeeks()` — Mulberry32 + Box–Muller | `3141` | 52 weeks. Temperature follows a sine year (14 °C ± 9) plus noise (sd 2.2). Both counts are log-linear in temperature so they stay positive at every temperature and nothing is floored: ice-cream tubs = exp(5.0 + 0.10·(temp − 14) + noise sd 0.25), about 148 at 14 °C; sunburn visits = exp(2.2 + 0.09·(temp − 14) + noise sd 0.30), about 9 at 14 °C. Temperature is the common cause; season is derived from temperature. |
...
- Framing: Group A mean 72.5, Group B mean 71.4, final-term difference 1.8 points; the drawn gap fills 1.8 % of the axis height on a 0–100 axis and 45 % on a 70–74 axis (25× taller, at any screen size). The axis minimum control stops at 70 and the maximum starts at 74, so no data value (71.0–73.6) can ever be pushed off the chart.
- Population: mean sleep 7.30 h; 25.2% late-library (302 of 1,200); late-library mean 6.34 h vs 7.62 h for others. At the library door the sampler design gives late-library students a 57.4% share (302 ÷ (302 + 898⁄4)); draw #1 at n = 500 shows 56.4%.
- Correlation: r = 0.81 across 52 weeks; within seasons r = 0.26 (cool, 20 weeks), 0.10 (mild, 18), 0.31 (warm, 14); temperature correlates 0.88 with ice cream and 0.85 with sunburn. Ranges: 41–637 tubs, 3–35 visits; no week is zero.
